...
of all agents. In there farmers are blue, bandits are orange and soldiers are
...
agents show the amount of resorces they have as a label on top of them. And the
red squares represent the danger zones, which are directly proportional to the
intencity of the color.
...
The second element of the interface are the pupulation and economic indicators,
which are shown in figure \ref{imp:population_plot1} and
figure \ref{imp:economy1} respectively. The show the number of individuals per
social group during all the execution and the total inventory that a specific
group has stored in their respective buildings (houses for farmers and bandits,
and city halls for soldiers).
...
sliders and the see danger zones switch that allow the user to see or hide the
danger messure in all patches represented with the intencity of the red color.
